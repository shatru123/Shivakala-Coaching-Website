...
public interface IPortalUserService
{
    ...
    Task<AppUser?> FindStudentUserAsync(string login, CancellationToken ct = default);
    Task<AppUser?> FindMpscUserAsync(string login, CancellationToken ct = default);
    ...
    Task<AppUser?> EnsureStudentAccountAsync(int studentId, string? username = null, string? password = null, CancellationToken ct = default);
